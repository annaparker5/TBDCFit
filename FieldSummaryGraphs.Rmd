...
save graph: 
```{r}
ggsave("~/Desktop/GitHub/TBDCFit/Figures/Fig4fieldnew.png", fieldfitnessplotsubset, width = 8, height = 5)
```
...
GRCfitnessplotB <- ggplot(aes(x = tot.area, y = rel.fitness), data = ffitnesssubset) + 
  ...
  geom_text(aes(x = 6000, y = 7.25, label = "B"), size = 10, color = "gray40") + 
  theme(legend.position = "none")
...
save graph: 
```{r}
ggsave("~/Desktop/GitHub/TBDCFit/Figures/GRCFieldnoCI.png", GRCfitnessplotB, width = 8, height = 5)
```
...
```{r}
fmodfitnesssubset2 <- lm(rel.fitness ~ treatment*type, data = ffitnesssubset)
