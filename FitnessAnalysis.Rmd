...
DCseedout <- DCseedout %>%
  select(pid, type, treatment, tot.seed.set)
...
TBseed <- TBseed %>%
  select(PID, type, treatment, binary, tot.seed.set)
...
comb <- merge(comb, DCseedout, by.x = c("PID", "treatment", "type", "column"), by.y = c("pid", "treatment", "type", "column"), all.x = TRUE)
comb <- merge(comb, TBseed, by.x = c("PID", "treatment", "type", "column"), by.y = c("PID", "treatment", "type", "column"), all.x = TRUE)
...
comb2$rel.fitness <- comb$prop.sprouted * comb$ln.stand.seed.set
```
