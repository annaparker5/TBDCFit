...
Remove plants for which we removed the TB pods
...
comb$rel.fitness <- comb$prop.sprouted * comb$stand.seed.set
```
...
```{r}
modseedset <- lmer(stand.seed.set ~ area.leaf*type + (1|column), data = comb)
...
```{r}
ggplot(data = comb, aes(x = area.leaf, y = stand.seed.set,  color = treatment)) + 
  geom_point(aes(shape = type)) + 
  theme_bw() + 
  scale_color_manual(values = c("#1E88E5", "lime green", "orange", "#D81B60")) + 
  scale_shape_manual(values = c(1, 4)) + 
  ylim(0,20) + 
  labs(x = "Leaf area consumed (m2)", y = "Standardized seed set", color = "Treatment group",
       shape = "Hostplant")
```


```{r}
ggplot(data = comb, aes(x = area.leaf, y = prop.sprouted,  color = treatment)) + 
  geom_point(aes(shape = type))  + 
  theme_bw() + 
  scale_color_manual(values = c("#1E88E5", "lime green", "orange", "#D81B60")) + 
  scale_shape_manual(values = c(1, 4)) + 
  labs(x = "Leaf area consumed (m2)", y = "Proportion of seeds sprouted", 
       color = "Treatment group", shape = "Hostplant")
```

```{r}
ggplot(data = comb, aes(x = area.leaf, y = rel.fitness,  color = treatment)) + 
  geom_point(aes(shape = type)) + 
  theme_bw() + 
  scale_color_manual(values = c("#1E88E5", "lime green", "orange", "#D81B60")) + 
  scale_shape_manual(values = c(1, 4)) + 
  ylim(0, 10) + 
  labs(x = "Leaf area consumed (m2)", y = "Relative fitness", color = "Treatment group",
       shape = "Hostplant")
```
