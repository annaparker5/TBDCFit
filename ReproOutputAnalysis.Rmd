...
Remove plants that didn't germinate at all
...
```{r}
ggplot(data = comb, aes(x = area.leaf, y = ln.stand.seed.set)) + 
...
```{r}
ggplot(data = comb, aes(x = area.leaf, y = prop.sprouted, shape = type)) + 
  geom_smooth(aes(linetype = type), color = "black", method = "lm") + 
...
```{r}
ggplot(data = comb, aes(x = area.leaf, y = rel.fitness)) + 
  #geom_smooth(method = "lm", color = "black") + 
  geom_point(aes(shape = type,  color = treatment)) + 
  theme_bw() + 
  scale_color_manual(values = c("#1E88E5", "lime green", "orange", "#D81B60")) + 
